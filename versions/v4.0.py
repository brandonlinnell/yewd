"""
    ...
    + Fully implemented Dashboard functionality for consultation data
    + Added cancel consultation route to allow users to cancel consultations
    + Added schedule request for scheduling installation/maintenance with validation
    + Introduced /api/consultations to fetch consultation data
    + Updated /api/products to handle product data within the dashboard
    + Modified consultation submission to use product_type instead of product_id
        + User is now sent to dashboard upon submitting consultation
    + Updated carbon footprint data to recent gov.uk figures (via content assets log)
    + Enhanced validation for scheduling maintenance, installations
    + Finalised UI/UX for client side dashboard HTML/CSS/JS
"""
...
def schedule_request():
    ...
    if not consultation_id or not schedule_date or not service_type:
        return jsonify({"success": False, "error": "Consultation ID, schedule date, and service type are required"}), 400
